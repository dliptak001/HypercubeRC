...
> [!WARNING]
> **HypercubeRC has been renamed to [HypercubeESN](https://github.com/dliptak001/HypercubeESN).**
> This project is no longer maintained — `0.2.5` is the final `hypercube-rc` release.
> Install the drop-in successor instead:
> ```bash
> pip install hypercube-esn
> ```
> and update imports from `hypercube_rc` to `hypercube_esn`.
