...
.. deprecated:: 0.2.5
    ``hypercube-rc`` has been renamed to **hypercube-esn** and is no longer
    maintained. Install the drop-in successor instead::

        pip install hypercube-esn

    and update imports from ``hypercube_rc`` to ``hypercube_esn``. This is the
    final ``hypercube-rc`` release.
...
warnings.warn(
    "hypercube-rc has been renamed to hypercube-esn and is no longer "
    "maintained. Install the drop-in successor with `pip install hypercube-esn` "
    "and import `hypercube_esn` instead. This is the final hypercube-rc release.",
    DeprecationWarning,
    stacklevel=2,
)
__all__ = ["ESN"]
