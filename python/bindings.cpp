...
void bind_esn(py::module_& m, const char* name)
{
    ...
    py::class_<E>(m, name)
        ...
        .def("train", [](E& self,
            ...
            } else if (!lambda_.is_none()) {
                // Ridge with custom lambda
                self.Train(ptr, n, lambda_.cast<double>());
            } else {
                // Default parameters
                self.Train(ptr, n);
            }
            ...
            "Ridge: pass lambda_ for custom regularization.\n"
        ...
        .def("predict_raw", &E::PredictRaw, py::arg("timestep"),
